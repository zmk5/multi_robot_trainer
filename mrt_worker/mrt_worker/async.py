"""Worker node for training (ASYNC).
...
class WorkerAsync(WorkerBase):
    ...
    def pull(self, executor: Executor) -> None:
        ...
        self.pull_server_parameters(executor)
        self.get_logger().info('Sent for weights!')

        # Once srv response received, set weights for policy
        self.get_logger().warn('Waiting for weights...')
        result = self._future_weights.result()
        self.get_logger().info('New weights recieved!')
        self._policy.parse_and_set_policy_weights(result)

        # Make target network same as other network only if episode 0.
        if self.episode == 0:
    ...
    def compute(self, step: int) -> None:
        ...
        if self.episode >= self._wp.training_delay:
            ...
            # batch = self._db.sample_batch(16)
    ...
    def pull_server_parameters(self, executor: Executor) -> None:
        ...
        rclpy.spin_until_future_complete(self, self._future_weights, executor)

    def push_gradients(self, executor: Executor) -> None:
        """Push neural net gradients TO server ROS node."""
        request = self._policy.transfer_gradients(Gradients.Request())
        request.id = self._worker_id
        self._future_gradients = self._cli['gradients'].call_async(request)
        rclpy.spin_until_future_complete(self, self._future_gradients, executor)
...
def main():
    ...
    node = WorkerAsync(worker_id, 'worker_node', policy_type)
