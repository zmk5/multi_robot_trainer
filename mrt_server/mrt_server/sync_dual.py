...
class ServerSyncDual(Node):
    ...
    def _weights_callback(
        ...
        if self._episodes[request.name][request.id] < self._current_episode:
            self.get_logger().info(
            ...
            return self._policy.set_response_weights(
    ...
    def _synchronization_callback(self) -> None:
        ...
        if (np.all(self._episodes['actor'] == self._current_episode)
                and np.all(self._episodes['critic'] == self._current_episode)):
            self._current_episode += 1
...
def main():
    ...
    node = ServerSyncDual('server_node', n_workers, output_file, policy_type)
